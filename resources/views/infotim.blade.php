        ...
        <div class="collapse navbar-collapse" id="navbarResponsive">
          <ul class="navbar-nav ml-auto">
            <li class="nav-item mx-0 mx-lg-1">
              <a class="nav-link py-3 px-0 px-lg-3 rounded js-scroll-trigger" href="{{route('tabeltimrha')}}">Kembali</a>
            </li>
             <li class="nav-item mx-0 mx-lg-1">
...
?>
    ...
    <section class="portfolio" id="portfolio">
      <div class="container">
        <h2 class="text-center text-uppercase text-secondary mb-0">Info Tim</h2>
        
        </br>
</br>
</br>
</br>
<?php $tims = $data->jenis_tim." - ".$data->nama_tim;
$i = 1;
?>
        <table class="table2 " style="align-self: center !important; " >
            <tr><td>ID Tim:</td><td>{{ $data->id_tim }}</td></tr>
             <tr><td>Nama Tim:</td><td>{{ $data->nama_tim}}</td></tr>
            <tr><td>Jenis Tim    :</td><td> {{ $data->jenis_tim }}</td></tr>
            <tr><td>Anggota     : </td> <td> 
              @foreach($relawan as $key => $datas)
                @if($datas->tim==$tims)
             {{$i}}. {{$datas->nama}} - {{$datas->keahlian}} </br>
                  <?php $i++ ?>
                   @endif
                @endforeach
              </td></tr>
