...
<html lang="en">
  ...
  <body id="page-top">
    ...
    <section class="portfolio" id="portfolio">
      <div class="container">
        <h2 class="text-center text-uppercase text-secondary mb-0">Daftar Tim Bantuan Kesehatan</h2>
                ...
                <table align="center" class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                  ...
                  <tbody>
                    @foreach($tim as $key => $datas)
                      @if($datas->jenis_tim=="Tim Bantuan Kesehatan")
                    <tr>
                        <td>{{ $datas->id_tim }}</td>
                        <td>{{ $datas->nama_tim }}</td>
                        <td>{{ $datas->bencana }}</td>
                        <td style="align-content: center; margin-left: 50px;"><form action="{{ route('tim.destroy', $datas->id_tim) }}" method="post">
                                {{ csrf_field() }}
                                {{ method_field('DELETE') }}
                                <a href="{{route('tim.info', $datas->id_tim )}}" class=" btn btn-sm btn-info">Info</a>
